...
- `vendor_emulator`: the workload ran on a vendor emulator that both applies a noise model of that vendor's hardware and compiles to that vendor's native gate set. Both conditions are required, and the row must carry an `execution_evidence` line naming the emulator, the noise model, and the gate set. Noiseless simulation, generic depolarising noise, and abstract gate sets do not qualify.
